...
def calculate_sleep_debt_impact(user_id: int, db: Session) -> dict:
    ...
    # Fetch all coding sessions for these logs at once
    log_ids = [log.id for log in logs]
    all_sessions = db.query(ActivitySession).filter(
        ActivitySession.log_id.in_(log_ids),
        ActivitySession.category == 'code'
    ).all()
    
    sessions_by_log = {log_id: [] for log_id in log_ids}
    for s in all_sessions:
        sessions_by_log[s.log_id].append(s)

    def get_productivity(log):
        sessions = sessions_by_log.get(log.id, [])
